...
//
// The `dump` function below is GIVEN and working, except for one.
...
void dump(const Memory& mem) {
    ...
    for (std::size_t row = 0; row < MEM_SIZE; row += BYTES_PER_LINE) {
        ...
        for (std::size_t col = 0; col < BYTES_PER_LINE; ++col) {
            Byte b = mem.data[row + col];
            //(lab-01, M2): when the byte IS printable, print the byte
...
void show_byte(Byte b) {
    //(lab-01, M3): print these four things on one line, separated by
